...
* **지능형 RAG (Nice-to-Have)**: 검색된 문서의 관련성 체크(Groundedness Check) 및 쿼리 재작성(Query Rewriting)을 통한 RAG 품질 향상 (향후 과제).

### **3.3. 학습 및 최적화 (Learning & Optimization)**
* **포트폴리오 구성**: 에이전트의 견해와 시장 데이터를 결합하여 최적의 자산 배분 비중을 계산해야 한다.
    - **통계적 결합 (Black-Litterman)**: 에이전트 위원회의 정성적 견해($Q_{raw}$)와 불확실성($\Omega_{raw}$)을 통계적 프레임워크를 통해 시장 균형과 결합한다.
    - **입력값 스케일링**: 에이전트의 투표 결과($-1 \sim +1$)를 실제 기대 수익률 단위로 변환하기 위해 상수($max\_view\_return$, 예: 0.05)를 사용한다.
        - $Q_{scaled} = Q_{raw} \times 0.05$
        - $\Omega_{scaled} = \Omega_{raw} \times 0.05^2$
* **성과 피드백**: 실제 시장 결과와 예측을 비교하여 에이전트의 판단을 평가해야 한다.
* **메모리 진화**: 평가 결과에 따라 유용한 정보는 상위 계층으로 승격시키고, 오답에 대해서는 회고 데이터를 생성하여 지식 베이스를 갱신해야 한다.

### **3.4. 자율 진화 및 메모리 갱신 (Self-Evolution & Memory Updates)**
에이전트는 실제 시장 수익률(`actual_return`)과 자신의 예측을 비교하는 회고(Reflection) 과정을 통해 지식의 중요도를 동적으로 조정한다. 이 과정은 FinMem 논문의 핵심 메커니즘을 따른다.

* **회고 프로세스 (Reflection Process)**: 
    - 에이전트 위원회의 판단($Q$)과 실제 시장 결과($Return$)의 방향이 일치하는지 평가하여 피드백($F$)을 생성한다.
    - $F = 1$ (예측 성공), $F = -1$ (예측 실패), $F = 0$ (중립).
    - 에이전트는 회고 답변 시 판단에 근거가 된 문서 ID를 명시해야 한다.

* **접근 카운터 (Access Counter, $AC$)**:
    - 각 문서에 대해 $AC$는 누적 합계(Running Total)로 관리된다.
    - 회고 과정에서 인용된 모든 문서에 대해 $AC_{new} = AC_{old} + F$ 를 수행한다.
    - 성공적인 예측에 반복적으로 기여한 문서는 높은 $AC$를 갖게 되며, 잘못된 판단을 유도한 문서는 $AC$가 감소한다.

* **중요도 점수 (Importance Score, $I$)**:
    - 접근 카운터를 기반으로 문서의 최종 중요도를 산출한다.
    - $I_{new} = I_{old} + (AC_{new} \times 5)$
    - 중요도는 0에서 100 사이로 캡핑(Capping)된다.
    - $AC$가 누적됨에 따라 중요도는 선형적으로 증가/감소하며, 이는 검색 시 **Importance-weighted Retrieval**에 반영되어 유용한 정보가 더 잘 검색되도록 한다.
